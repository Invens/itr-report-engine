...
export async function generateIndividualReport(input: IndividualReport) {
  ...
  const table = new Table({
    ...
    rows: [
      new TableRow({ children: [
        'S. No.', 'Acknowledgement No.', 'Date of Filing', 'Whether Original or Revised', 'A.Y.',
        'Total Income as per ITR', 'Total Tax Interest and fee payable as per ITR', 'Total Taxes Paid as per ITR'
      ].map((value) => cell(value, true)) }),
      ...data.rows.map((row, index) => new TableRow({ children: [
        String(index + 1), row.acknowledgementNumber, row.filingDate, row.filingType,
        row.assessmentYear, inr(row.totalIncome), inr(row.totalTaxInterestFeePayable), inr(row.totalTaxesPaid)
  ...
  const doc = new Document({ sections: [{
    ...
    children: [
      ...
      new Paragraph(''),
      new Paragraph('The Chief Manager,'), new Paragraph(`${data.bankName},`),
      new Paragraph(data.branchName), new Paragraph(data.address), new Paragraph(''),
      new Paragraph({ children: [new TextRun({ text: `Sub: - Report on Verification of ITR (Acknowledgement No.) of ${data.clientName.toUpperCase()}`, bold: true })] }),
      new Paragraph(''), new Paragraph('Dear Sir,'), new Paragraph(''),
      ...
      new Paragraph(''), new Paragraph('➢ The Details of ITR Provided to us is as under:'), new Paragraph(''),
      table, new Paragraph(''), new Paragraph('Submitted for information and necessary action.'),
      new Paragraph(''), new Paragraph('Thanking you,'), new Paragraph(''), new Paragraph('Yours faithfully,'),
      new Paragraph('For: Singhi Bikash & Associates'), new Paragraph('Chartered Accountants'),
      new Paragraph('Firm Regd. No. 020937N'), new Paragraph(''),
      new Paragraph('CA BIKASH SINGHI (B. Com, FCA, IP)'), new Paragraph('Partner'), new Paragraph('M. No: 098836')
    ]
  }] });
